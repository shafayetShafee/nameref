---
title: "Using nameref"
author: Shafayet Khan Shafee
date: last-modified
date-format: "DD MMM, YYYY"
number-sections: true
filters: 
  - quarto
  - nameref
nameref:
  section-number: true
format: pdf
df-print: kable
---

# Why Quarto is so great {#sec-stack}

`r stringi::stri_rand_lipsum(1)`. See \nameref{sec-stack}.

## Why Quarto is so great {#how}

`r stringi::stri_rand_lipsum(1)`. See \nameref{how}.

## Images

```{r}
#| link-id: fig1
#| link-title: My Awesome plot
#| label: fig-random-plot
#| fig-cap: "My Awesome plot"

plot(1:10)
```

`r stringi::stri_rand_lipsum(1)`. See @fig-random-plot \nameref{fig1}. And you can use quarto way to refer to @fig-random-plot. 

## Tables

```{r}
#| link-id: tab1
#| link-title: First five row of mtcars data
#| label: tbl-mtcars
#| tbl-cap: First five row of mtcars data

head(mtcars)
```

`r stringi::stri_rand_lipsum(1)`. See @tbl-mtcars \nameref{tab1}. And you can use quarto way to refer to @tbl-mtcars.


# Markdown Images


::: {.link link-id="fig2" link-title="Scatter plot"}

![mpg ~ hp](images/mpg.png){#fig-fig2}

:::

`r stringi::stri_rand_lipsum(1)`. See @fig-fig2 \nameref{fig2}. And you can use quarto way to refer to @fig-fig2.


# Markdown Table


::: {.link link-id="tab2" link-title="Markdown table"}

| Col1 | Col2 | Col3 |
|------|------|------|
| A    | B    | C    |
| E    | F    | G    |
| A    | G    | G    |

: My Caption {#tbl-tab2}

:::

`r stringi::stri_rand_lipsum(1)`. See @tbl-tab2 \nameref{tab2}. And you can use quarto way to refer to @tbl-tab2.